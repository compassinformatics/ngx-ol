...
export class ReactiveSources {
  ...
  readonly tileJsonTheme = signal<TileJsonTheme>('light');
  ...
  readonly tileJsonUrl = computed(() => `tile-json/carto-${this.tileJsonTheme()}.json`);
  ...
  protected setTileJsonTheme(theme: TileJsonTheme): void {
    this.tileJsonTheme.set(theme);
    this.sourceStatus.set(`TileJSON URL changed: ${this.tileJsonUrl()}`);
  }
